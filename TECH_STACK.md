...
Habit archival is non-destructive and transactional. Archiving timestamps the habit definition so it cannot produce current or future projections or occurrences, and marks an already-materialized occurrence on the archive date as excluded with an archival reason. Excluded occurrences remain stored for lifecycle integrity but do not appear in Today or contribute to that day's completion denominator. Earlier occurrence snapshots remain visible and retain their original contribution semantics regardless of the habit definition's current archived state. Destructive habit deletion is not an application operation.
...
Daily schedules are reconciled through the current civil date when the application starts or detects a day rollover. This materializes missed scheduled days as incomplete occurrences, allowing calendar history and contribution aggregation to distinguish a real zero-completion day from an unscheduled day. Future-date browsing projects the current definitions, schedules, and routine memberships through a read-only query; it must not materialize occurrence rows or contribution activity before that civil date begins. Contribution percentages are aggregated from dated occurrence rows in SQLite and rendered from view-ready application data; rendering code does not calculate persistence semantics.
